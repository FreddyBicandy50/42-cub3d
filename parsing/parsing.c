...
/*   Updated: 2025/07/17 16:44:19 by fbicandy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!check_texture(p->north_filename) || !check_texture(p->south_filename)
		|| !check_texture(p->west_filename) || !check_texture(p->east_filename))
	{
		p->error_message = "error with texture of map";
		return (0);
	}
	if (!check_color(p->ceiling_color, p) || !check_color(p->floor_color, p))
	{
